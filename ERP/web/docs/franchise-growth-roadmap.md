...
- SV/운영관리 2차 범위 중 2026-07-03에 기존 슈퍼바이징 탭 안에서 내부 탭 재구성, KPI 클릭 필터, 회사별 점검 템플릿 저장/적용, 보고서/시정요청 처리 이력, 방문 일정 수정 시 공용 일정 동기화, 점검 보고서 PDF/인쇄, 내부 알림톡 발송 훅의 1차 연결을 구현했다. 적용 SQL은 `supabase_franchise_supervision_v2_migration.sql`이며 사용자가 Supabase SQL Editor에 직접 등록해야 한다. **SQL 등록 필요**. 코드리뷰 후 승인/반려 상태 전환, 회사 범위 검증, 이벤트 RLS, 첨부 URL 신뢰 경계를 보강했다. 남은 2차-3은 방문 D-1/D-day와 보고서 미제출을 스케줄러/운영 큐로 자동 생성하고, 대표 대시보드 KPI와 지연/미제출 운영 큐를 더 깊게 연결하는 작업이다.
