...
- 2026-07-30 회사별 유입경로 항목 관리를 추가했다. 자동 수집·DB 승격에 쓰는 고정 항목은 수정하지 못하게 보호하고, 직접 관리 항목은 안정 코드를 유지한 채 표시 이름 변경·사용 중지·재사용을 지원한다. 다음 P1은 항목 순서 변경, 사용 중지 항목의 별도 필터 표시, 변경 이력 감사 로그이며 운영 SQL 적용과 실계정 저장 QA 후 우선순위를 정한다.
