...
- 2026-06-12 모객 DB route page의 UI 구조 분리를 시작했다. 후보자 등록 모달, 빠른 상담 이력 모달, 후보자 상세 패널, Meta 연동 설정 패널을 컴포넌트로 분리했으며, 다음 단계는 `page.tsx`에 남아 있는 Meta 동기화, 엑셀 업로드, 고객 전환, 후보지 연결 action을 feature hook으로 분리하는 것이다.
