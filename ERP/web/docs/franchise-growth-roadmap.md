...
- 점주 포털은 1차에서 별도 `/owner/login`/`/owner/dashboard`와 본사 `가맹 운영 > 점주 연동` 큐까지 구현한다.
  ...
  - 원칙: 점주는 `profiles`에 넣지 않고 점주 전용 계정/세션으로 자기 운영점 하나만 접근한다. 점주 제출 정보는 바로 핵심 운영 데이터를 덮어쓰지 않고 본사 승인 또는 검토 큐를 거친다.
