...
- 같은 출점 후보지나 외부 상가를 여러 후보자에게 중복 연결할 수 있다.
- 초기 구현은 기존 ERP 위치 DB와 저장된 외부 상가 DB만 사용한다. SearchAPI 유료 결제 전까지 외부 검색 API 의존성 없이 담당자 수동 매칭을 우선한다.
- 다음 단계는 연결된 후보지에서 출점 후보지 인사이트/경쟁스캔 상세로 이동하거나, 선택 외부 매물을 ERP 물건지로 승격하는 흐름이다.
...
- 모객 DB 업무 큐와 점포·상권 매칭을 로그인 세션에서 QA한다.
  - 업무 큐는 `전체 업무`, `연락 지연`, `오늘 연락`, `무응답` 기준으로 실제 목록과 건수가 맞는지 확인한다.
  - 후보자 상세에서 출점 후보지와 외부 상가 DB를 검색/연결/메모/삭제할 수 있는지 확인한다.
  - 같은 후보지를 여러 후보자에게 중복 연결할 수 있는지 확인한다.
  - 초기 매칭은 기존 ERP 점포/위치 DB와 저장된 외부 상가 DB를 사용하고, 유료 외부 검색 API 의존성은 두지 않는다.
