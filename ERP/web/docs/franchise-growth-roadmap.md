...
- 2026-06-16 요약 대시보드는 기존 그래프형 화면을 `B 타입` 기본값으로 유지하고, `A 타입`에는 정보공개서 발송 필요, D-3/D-1, 계약 가능, 연락 확인처럼 담당자 액션 중심의 운영 대시보드를 추가했다.
...
- 2026-06-16 `/admin/users`에서 사용자별 직급/권한을 관리자, 팀장/매니저, 담당자로 변경할 수 있게 했다. 본인 관리자 권한을 직접 낮추거나 마지막 관리자를 제거하는 변경은 API에서 차단한다.
