...
8. Meta Lead Ads 회사별 신청 질문 매핑은 구현 완료. 실광고 수신·Webhook/백필 안정성 QA와 광고 성과 Marketing API는 별도 권한 검수까지 HOLD

## 현재 완료/진행 상태

### 가맹운영 2단계 일정·알림 통합 코드 완료
...
- 2026-07-28 각 회사가 연결한 Meta 신청 양식의 실제 질문을 모객 DB 고정 항목에 연결하는 설정을 추가했다. 자동 추천, 질문별 수동 연결, `연결 안 함`, 저장 전 변경 보존, 이름·연락처·담당자 준비성 검사를 제공하며 재연결 시 운영 설정을 보존한다. 다음 P1은 회사·브랜드별 재사용 매핑 템플릿, 매핑 변경 이력, 과거 리드 재처리이며 실제 착수 시 별도 데이터 모델과 SQL 필요 여부를 검토한다.
- 2026-07-30 회사별 유입경로 항목 관리를 추가했다. 자동 수집·DB 승격에 쓰는 고정 항목은 수정하지 못하게 보호하고, 직접 관리 항목은 안정 코드를 유지한 채 표시 이름 변경·사용 중지·재사용을 지원한다. 사용자 확인 기준 운영 SQL 적용을 완료했다. 다음 P1은 항목 순서 변경, 사용 중지 항목의 별도 필터 표시, 변경 이력 감사 로그이며 실계정 저장·복원과 회사 간 격리 QA 후 우선순위를 정한다.
...
- 2026-06-30 Kakao 비즈니스 심사 대응을 위해 공개 화면 하단에 주식회사 내일사장 사업자정보 푸터를 추가했다. 대상은 `/landing`, `/login`, `/signup`, `/privacy`이며, 개인정보처리방침 문의 이메일도 `cs@sajang.app`로 맞췄다.
...
- 2026-07-02 입점요청의 신규 첨부는 Storage 업로드 후 `storageBucket`, `storagePath`, `publicUrl`을 함께 저장해 확인/수정 모달에서 이미지 썸네일과 PDF/문서 다운로드를 제공한다. 과거에 파일명/용량/타입만 저장된 첨부는 원본 파일을 복원할 수 없으므로 `재첨부 필요` 안내를 유지한다. 현재 업로드 API 허용 형식에 맞춰 입점요청/출점 후보지 첨부 UI의 노출 문구는 PDF/JPG/PNG/WebP 기준으로 정리했다.
...
- 2026-07-03 정보공개서 Gmail 발송 성공 안내 알림톡을 연결했다. 발송 전 담당자가 `후보자명`을 필수 입력하고 `정보공개서 확인 안내` 알림톡 목업을 확인한 뒤 Gmail을 발송한다. Gmail 발송 성공 시 후보자 휴대폰으로 `disclosure_email_sent` 알림톡을 보내고, 후보자가 메일의 수령 확인 버튼을 누르면 기존 `disclosure_confirmed` 알림톡과 14일 숙고기간 감사 기록을 유지한다. `disclosure_confirmed`는 승인 템플릿 변수 차이를 흡수하기 위해 `확인일`, `수령확인일`, `수령일`, `계약가능일`을 모두 제공한다. 메일 열람 추정은 Gmail/메일 클라이언트 프록시 때문에 법적 수령 신호로 쓰지 않고, 발송 또는 열람 추정 후 1일 이상 수령 확인이 없을 때 내부 `정보공개서 수령 미확인` 업무 큐를 생성한다. 실제 Gmail 발송 live QA는 Google OAuth 승인/연결 완료 이후 진행한다.
...
- SV/운영관리 1차 MVP는 `/dashboard/franchise-operations` 안의 `슈퍼바이징` 탭으로 시작했다. SV별 운영점 배정, 방문 점검 일정, 체크리스트 기반 점검보고서 저장/제출, 관리자 승인/반려, `개선필요` 항목의 시정요청 전환을 회사 범위로 관리한다.
- SV/운영관리 2차 범위 중 2026-07-03에 기존 슈퍼바이징 탭 안에서 내부 탭 재구성, KPI 클릭 필터, 회사별 점검 템플릿 저장/적용, 보고서/시정요청 처리 이력, 방문 일정 수정 시 공용 일정 파일럿 동기화, 점검 보고서 PDF/인쇄, 내부 알림톡 발송 훅의 1차 연결을 구현했다. 이 공용 일정 파일럿은 2단계 완료 범위에서 제외했고, 현재 SV 원천 일정은 `franchise_schedules`와 `/dashboard/franchise-operations/schedule`로 대체했다. 과거 파일럿 행의 이관·종료는 후속 운영 고도화 항목이다. 적용 SQL은 `supabase_franchise_supervision_v2_migration.sql`이며 사용자가 Supabase SQL Editor에 직접 등록해야 한다. **SQL 등록 필요**. 코드리뷰 후 승인/반려 상태 전환, 회사 범위 검증, 이벤트 RLS, 첨부 URL 신뢰 경계를 보강했다. 추가 고도화로 기존 테이블 기반 `운영 우선순위`를 운영 리포트에 연결해 오늘/내일 방문, 보고서 미제출, 승인 대기, 시정요청 지연을 기존 탭 필터로 바로 열 수 있게 했다. 운영 리포트는 팀장/관리자 관점과 SV 관점 문구를 분리하고, `배정 관리` 탭은 팀장/관리자에게만 노출한다. 배정 관리는 운영점별/SV별 목록, 필터, 페이지네이션, 선택 행 하단 인라인 수정/배정 폼으로 정리해 목록을 먼저 훑고 필요한 운영점만 열 수 있게 했다. 방문 일정은 목록 우선 화면에 검색/SV/상태 필터, 페이지네이션, 수정/삭제를 추가했고, 등록/수정 폼은 SV 선택 후 해당 SV에게 지정된 운영점만 표시한다. 점검 보고서 탭은 `보고서 목록`과 `보고서 작성`을 분리해 방문별 보고서 작성 상태와 개선필요/사진/특이사항을 비교한 뒤 선택 보고서를 작성한다. AI 회의록 정리는 항목별 적용/제외 검토 후 반영하고, PDF/인쇄에서는 조치 필요 항목 요약과 전체 점검 내역 상세를 분리해 중복 메모를 줄였다. 승인·시정요청 탭은 처리 목록으로 정리해 승인/반려와 시정요청 상태 변경을 더 빠르게 처리한다. 남은 2차-3은 방문 D-1/D-day와 보고서 미제출 알림톡을 스케줄러로 자동 발송하고, 대표 대시보드 KPI와 지연/미제출 운영 목록을 더 깊게 연결하는 작업이다. AI 고도화는 동기 버튼 UX에서는 빠른 요약 모델을 유지하고, 긴 회의록·사진/첨부 포함 고품질 재정리는 비동기 작업으로 분리한다.
...
- 인사·노무는 `가맹 운영 > 인력 세팅` 탭에서 시작한다. 2026-07-07 1차로 운영점별 매출 규모 기반 권장 인력 세팅, 주간 근무표, 급여/3.3%/일당 계산기, 근로계약서 전자서명 서식함 진입점을 추가했다. 저장/이력은 `supabase_franchise_labor_planning_migration.sql` 적용 후 활성화된다. 2차는 노무 서식함과 전자계약 템플릿 변수 연동, 3차는 오픈 준비/계약완료 상세의 `인력 세팅 열기` 바로가기, 4차는 실제 직원 근무표·급여대장·알림톡/문서 발송으로 확장한다. 점주 상담과 본사 인사 업무가 같은 계산기를 쓰는 구조를 목표로 한다. **SQL 등록 필요**.
- 점주 포털/점주 소통은 모바일 친화 화면으로 두되, 본사 운영 데이터 구조가 안정된 뒤 확장한다. 필수 매장 정보인 보증금, 월세, 관리비, 평수, 테이블 수, 좌석 수는 출점 후보지/운영점 데이터와 연결되는 장기 입력값으로 관리한다.
- 프랜차이즈 업무 일정은 `가맹 운영 > 일정관리`를 단일 허브로 사용한다. 정보공개서 계약 가능일, SV 점검, 업체 계약 만료, 오픈 준비, 점주 문의와 체크리스트 후속 업무는 `franchise_schedules`에만 저장하고 담당자와 관리자에게 알림을 보낸다. 점포개발 업무의 일정관리와 `schedules`는 다른 사용자 영역이므로 프랜차이즈 원천 일정과 연결하지 않는다. 전자결재 요청은 일정이 아니라 헤더 알림과 전자결재 결재함에서 처리한다.
...
- `/dashboard/franchise-operations`는 운영 가맹점 마스터를 중심으로 본사 운영 현황을 본다. 2026-07-02 기준 화면은 `대시보드 / 슈퍼바이징 / 가맹점 목록 / 가맹점 등록`으로 분리해, 운영 현황, SV 점검, 목록, 등록 폼이 섞이지 않게 정리했다.
...
- 오픈 준비 프로젝트 UI는 가맹 운영 화면에서는 숨김 유지하고, 계약완료 점주 상세의 `오픈 준비` 탭으로 이관했다. 기존 `franchise_opening_projects` 테이블/API를 재사용하며, 프로젝트는 계약 lead가 아니라 해당 lead에서 생성된 `franchise_locations.id` 기준으로 저장한다. 가맹점 정보가 없으면 `가맹점 정보` 탭 이동 CTA를 먼저 보여주고, `오픈준비` 상태의 가맹점에서만 프로젝트 시작/저장을 허용한다. 2026-06-24 1차 고도화로 기존 6개 항목을 `계약/행정`, `인테리어`, `교육`, `초도물류`, `홍보`, `오픈일` 6단계 25개 하위 체크로 확장하고, `확인요청`, 오늘 처리, 기한 임박, 진행 이슈, 오픈 가능도 요약을 추가했다. 신규 SQL은 없다.
...
- 2026-07-01 업체 계약함 MVP를 추가했다. `/contracts/vendor`에서 물류, 식자재, 인테리어, 마케팅, 임대차, 기타 업체 계약을 회사 범위로 관리하고, 파일 업로드 문서 또는 같은 회사 전자계약 문서를 연결한다. 계약 만료 D-30/D-7은 기존 `franchise_notifications` 인앱 알림으로 담당자와 팀장에게 동기화한다. 적용 SQL은 `supabase_franchise_vendor_contracts_migration.sql`이며, 증거 묶음 PDF 출력은 현재 운영 필요도가 낮아 제외했다.
- 2026-07-01 업체 계약함 2차-2A를 추가했다. `supabase_franchise_vendor_contract_events_migration.sql`로 계약 lifecycle 이벤트를 저장하고, `/api/franchise-vendor-contracts/actions`에서 갱신/종료 처리를 제공한다. 화면은 만료 업무 큐, 계약 상세 패널, 처리 이력을 보여준다. 갱신은 원본 계약을 `renewed`로 닫고 새 active 계약을 복사 생성하며, 종료는 사유를 남기고 `terminated`로 처리한다.
- 2026-07-01 업체 관리 별도 메뉴를 추가했다. `/dashboard/franchise-vendors`는 `franchise_vendors` 업체 마스터를 직접 등록/수정하고, 계약함에서 업체 마스터를 선택한 계약은 `vendor_id` 기준으로 우선 병합한다. 기존 직접입력 계약은 업체명 fallback으로 묶는다. 업체 목록 안의 `업체 생성` 버튼으로 계약 전 거래처를 먼저 만들 수 있으며, 화면은 업체별 계약 수, 진행/관리 필요 건수, 다음 만료 계약, 최근 메모를 보여주고 `계약 보기`로 업체 계약함 검색 상태에 연결한다.
...
- 업체 계약함 다음 후속은 업체 마스터 등록 화면을 실제 계약 데이터로 live QA한 뒤, 외부 알림톡/문자/이메일 발송 로그, 공용 달력/대표 대시보드 projection 순서로 검토한다. 담당자 변경 후에도 계약 원본과 만료일은 회사에 남기고, 업체 마스터는 연락처/사업자번호/거래 상태처럼 계약과 분리된 기본 거래처 정보만 담당한다.
...
- 2026-06-18 기존 개인별 유캔싸인 계약 기능은 유지하고, 별도 `/contracts/electronic` 권리금 전자계약 v2를 추가했다. 신규 흐름은 내일사장 공용 UCanSign API KEY 1개로 발송하고 ERP의 `company_id`, `sent_by_profile_id`로 회사 문서와 내가 발송한 문서를 분리한다. 적용 SQL은 `supabase_electronic_contracts_platform_migration.sql`이며, 인허가번호는 SafetyData 전체 수집 후 내부 `license_business_records`에서 조회한다.
...
- 알림톡 3차 계획은 운영 자동화와 분석 단계다. 시나리오별 발송량/실패율/월 한도 초과 위험을 대표 대시보드와 관리자 화면에 연결하고, 회사별 발송 사용 정책, 템플릿 변수 미리보기, 재발송/수동 발송, 실패 원인 필터, 알림톡 승인 상태 점검, 공용 달력/업체계약 만료 큐 연동을 검토한다. 이 단계에서 메시지 예약, 재시도 정책, provider webhook 수신, 비용 집계, 회사별 과금 또는 사용량 리포트가 필요해지면 별도 SQL을 작성한 뒤 사용자가 Supabase SQL Editor에 직접 등록한다.
...
- 2026-06-29 점포개발 미팅 도구 1차로 출점 후보지 목록에 `출점 검토 리포트`를 추가했다.
  - 후보지별 목표매출, 재료비/인건비/관리비·공과금/기타잡비/로열티 금액과 비율을 만원 단위로 계산하고, 세전수익/세전 수익률을 즉시 확인한다.
  - 목표매출 변화는 `1차 / 2차 / 3차`로 저장하고 전환 시 현재 비용 항목의 비율을 재계산한다.
  - 미팅 중 필요한 자유 비용 항목은 리포트 안에서 추가/삭제할 수 있다.
  - 기존 `franchise_locations.data.meetingTool`에 저장하므로 신규 SQL은 없다.
  - 브라우저 인쇄/PDF 저장으로 미팅 보고서를 출력한다.
  - 2026-06-30 2차-1/2차-4 묶음으로 출력물 레이아웃을 목표매출 1~3차 비교표, 비용 구조, 검토 의견, 내부 검토 안내 중심의 미팅 자료형 PDF/인쇄 구성으로 고도화했다. 후보지별 리포트 버전 이력은 `franchise_location_meeting_tool_versions`에 snapshot을 저장하고, 현재안 버전 저장/이전안 불러오기를 제공한다.
  - 2026-06-30 2차-5로 `상권분석·목표매출 근거` 섹션을 추가했다. 상권 요약, 수요 근거, 목표매출 산정 근거, 리스크/확인사항은 후보지별 `meetingTool.marketReport`에 저장하고, PDF/인쇄 출력물과 버전 snapshot에 포함한다. 회사 공용 프리셋에는 포함하지 않는다.
  - 2026-07-01 Kakao 상권 지도 섹션을 추가했다. 후보지 좌표가 있으면 바로 지도와 마커/반경을 표시하고, 좌표가 없으면 주소 지오코딩을 시도한다. 반경은 300m/500m/1km 중 선택해 후보지별 `meetingTool.marketMap.radiusMeters`에 저장한다. 지도에는 확대/축소, 일반/스카이뷰/지적편집도 전환, 거리재기, 면적재기 도구를 제공한다. 측정 점은 후보지별 `meetingTool.marketMap.measurementPoints`에 저장하고, PDF/인쇄 출력물에는 Kakao SDK를 새 출력창에서 로드해 지도 타일, 마커, 반경 원, 측정 선·면·점을 표시한다. 별도 좌표 기준 박스는 출력하지 않는다. 출력물의 상권분석·목표매출 근거는 비용 구조 아래에 배치하고, 인쇄 카드가 페이지 경계에서 잘리지 않도록 보강했다. 회사 공용 프리셋에는 포함하지 않는다.
  - 다음 고도화는 회사/브랜드별 기본 비용 항목 라이브러리와 브랜드별 기본 원가율 템플릿으로 분리한다.
...
- 점주 포털은 1차에서 별도 `/owner/login/{companyId}`/`/owner/dashboard`와 본사 `가맹 운영 > 점주 소통`의 공지/공문, 운영 체크리스트, 제출 처리, 점주 계정 설정까지 구현했다. 기존 `/owner/login` 회사명 입력 방식과 `/owner/login?companyId=...` 쿼리 링크는 호환용으로 유지한다.
  - 운영 체크리스트는 공지처럼 본사가 전체/복수 운영점에 발송하고, 점주 화면에서는 발송 1건 목록과 접힘 상세로 표시한다. 본사 발송 현황은 체크리스트별 목록과 가맹점별 완료/미완료 그리드로 확인한다.
  - 2차: 교육자료/매뉴얼 라이브러리, 정산/영수증 제출, 알림톡 발송을 추가한다.
  - 3차: 전자계약/시정요청서/공문 수령 확인, PWA 홈 화면 설치, 점주별 활동 리포트를 연결한다.
  - 4차: 점주용 계약/문서함, 자동 리마인드, 본사 승인 워크플로우를 고도화한다.
  - 원칙: 점주는 `profiles`에 넣지 않고 점주 전용 계정/세션으로 자기 운영점 하나만 접근한다. 점주 제출 정보는 바로 핵심 운영 데이터를 덮어쓰지 않고 본사 처리/검토 단계를 거친다.
