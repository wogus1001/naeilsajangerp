...
- 업무 큐 기준은 연락 지연, 오늘 연락, 무응답, 계약 가능, 즉시상담이다.
...
- 이 필드는 다음 `점포·상권 매칭`에서 후보자별 추천 점포/추천 지역/예산 적합도 계산의 입력값으로 사용한다.

### 점포·상권 매칭

- 후보자 상세 패널에 기존 `franchise_locations` 기반 추천 점포/후보지 카드를 추가한다.
- 1차 점수 기준은 희망지역 일치, 관심브랜드 일치/유사, 후보지 상태, 업무 큐의 자금/지역/브랜드 적합도, 경쟁업체 수 리스크다.
- 초기 구현은 기존 ERP 위치 DB만 사용한다. SearchAPI 유료 결제 전까지 외부 검색 API 의존성 없이 추천 근거를 보여준다.
- 다음 단계는 추천 카드에서 출점 후보지 인사이트/경쟁스캔 상세로 이동하거나, 선택한 후보지를 상담 이력에 연결하는 흐름이다.
...
- 모객 DB 업무 큐 강화 후 점포·상권 매칭을 진행한다.
- 모객 DB의 `1차 유입 DB -> 후보자` 승격 흐름을 로그인 세션에서 QA한다.
  - Meta/엑셀 유입은 1차 유입 DB에 쌓이는지 확인한다.
  - 후보자 승격 후 파이프라인/업무 큐/점포·상권 매칭 대상이 되는지 확인한다.
  - 기존 단계값 없는 리드는 후보자로 유지되는지 확인한다.
- 모객 DB 업무 큐와 점포·상권 매칭을 로그인 세션에서 QA한다.
  - 후보자의 희망지역, 예산, 관심브랜드, 업무 큐 적합도 필드를 기준으로 추천 점포/추천 지역 후보를 보여준다.
  - 후보자 상세에서 추천 근거와 리스크를 확인할 수 있게 한다.
  - 초기 매칭은 기존 ERP 점포/위치 DB를 사용하고, 유료 외부 검색 API 의존성은 두지 않는다.
...
- 외부 상가 수집 MVP를 실제 Supabase migration 적용 후 검증한다.
  ...
  - 화면 2000건 수집 요청, import API 3000건 안전 상한, 저장 목록 API 2000건 상한이 의도대로 동작하는지 확인한다.
