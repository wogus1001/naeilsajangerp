...
- 남은 P0 회귀 QA는 실제 Meta/엑셀 업로드 파일 유입과 여러 회사/권한 계정 범위 확인이다.
...
- 외부 상가 수집 MVP를 실제 Supabase migration 적용 후 검증한다.
  ...
  - 선택 외부 상가 수동 승격은 2026-06-11 1차 구현/QA와 점포목록 상세/검색/외부수집 필터 회귀 확인을 통과했다. 다음은 운영 화면에서 승격 물건지 표시를 회귀 확인한다.
- 외부 상가 수집 고도화는 아래 순서로 진행한다.
  ...
  - 6순위: 사용자가 선택한 외부 매물만 ERP 물건지로 승격 완료. 점포목록 상세/검색/외부수집 필터 회귀 QA는 통과했으며, 다음은 권한 회귀 QA와 승격 후 운영 워크플로 연결
