...
- 남은 P0 회귀 QA는 실제 Meta/엑셀 업로드 파일 유입, 기존 단계값 없는 리드 유지, `연락 완료` 처리, 후보지 연결 상태 변경 저장, 여러 회사/권한 계정 범위 확인이다.
...
- 외부 상가 수집 MVP를 실제 Supabase migration 적용 후 검증한다.
  - 2026-06-11 `서울 광진구 화양동` 상가 수집은 통과했다. 다음은 당근 `합정동`, `광진구` 구 단위 대량 수집 결과를 확인한다.
  ...
  - 선택 외부 상가 수동 승격은 2026-06-11 1차 구현/QA를 통과했다. 다음은 점포목록 상세/검색과 운영 화면에서 승격 물건지 표시를 회귀 확인한다.
- 외부 상가 수집 고도화는 아래 순서로 진행한다.
  ...
  - 6순위: 사용자가 선택한 외부 매물만 ERP 물건지로 승격 완료. 다음은 점포목록 상세 편집/검색/권한 회귀 QA와 승격 후 운영 워크플로 연결
