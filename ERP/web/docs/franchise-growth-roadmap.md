...
- SV/운영관리 2차 범위 중 2026-07-03에 기존 슈퍼바이징 탭 안에서 내부 탭 재구성, KPI 클릭 필터, 회사별 점검 템플릿 저장/적용, 보고서/시정요청 처리 이력, 방문 일정 수정 시 공용 일정 동기화, 점검 보고서 PDF/인쇄, 내부 알림톡 발송 훅의 1차 연결을 구현했다. 적용 SQL은 `supabase_franchise_supervision_v2_migration.sql`이며 사용자가 Supabase SQL Editor에 직접 등록해야 한다. **SQL 등록 필요**. 코드리뷰 후 승인/반려 상태 전환, 회사 범위 검증, 이벤트 RLS, 첨부 URL 신뢰 경계를 보강했다. 추가 고도화로 기존 테이블 기반 `오늘 처리 큐`를 운영 리포트에 연결해 오늘/내일 방문, 보고서 미제출, 승인 대기, 시정요청 지연을 기존 탭 필터로 바로 열 수 있게 했다. 배정 관리는 운영점별/SV별 목록, 필터, 페이지네이션, 선택 행 하단 인라인 수정/배정 폼으로 정리해 목록을 먼저 훑고 필요한 운영점만 열 수 있게 했다. 점검 보고서 탭은 목록 리스트형 운영 화면으로 확장해 방문별 보고서 작성 상태와 개선필요/사진/특이사항을 비교한 뒤 선택 보고서를 작성한다. 승인·시정요청 탭은 처리 큐 테이블로 정리해 승인/반려와 시정요청 상태 변경을 더 빠르게 처리한다. 남은 2차-3은 방문 D-1/D-day와 보고서 미제출 알림톡을 스케줄러로 자동 발송하고, 대표 대시보드 KPI와 지연/미제출 큐를 더 깊게 연결하는 작업이다.
