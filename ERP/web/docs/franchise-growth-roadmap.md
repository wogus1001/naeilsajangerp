...
- 수집 결과는 실행 요약으로 보여주고, 하단 `저장된 상가` 목록에 누적 저장한다. 저장 목록은 저장 시군구 칩, 동 단위 카드, 동 내부 페이지네이션, 저장일, 별표, 추천점수, 별표/1층/관리비 필터, 정렬을 제공한다. `최신화`는 같은 매물을 중복 추가하지 않고 새로 발견된 매물만 신규 저장한다.
