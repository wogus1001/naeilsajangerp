...
- 2026-07-03 정보공개서 Gmail 발송 성공 안내 알림톡을 연결했다. 발송 전 담당자가 `후보자명`을 필수 입력하고 `정보공개서 확인 안내` 알림톡 목업을 확인한 뒤 Gmail을 발송한다. Gmail 발송 성공 시 후보자 휴대폰으로 `disclosure_email_sent` 알림톡을 보내고, 후보자가 메일의 수령 확인 버튼을 누르면 기존 `disclosure_confirmed` 알림톡과 14일 숙고기간 감사 기록을 유지한다. 실제 Gmail 발송 live QA는 Google OAuth 승인/연결 완료 이후 진행한다.
