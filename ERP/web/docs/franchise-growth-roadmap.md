...
- 등록 폼의 1차 핵심 입력 섹션은 `기본 위치`, `물리 조건`, `점포구입비`, `임차조건`, `임대인`, `종합메모`로 둔다. 운영점 마스터에 가까운 `오픈일` 중심 입력보다, 후보 입점지 검토에 필요한 조건 입력을 앞에 둔다.
- 점포 조건 필드는 `실평수`, `화장실 여부`, `엘리베이터 여부`, `철거 여부`, `주차 여부`를 기본으로 두고, 각 항목에 현장 메모를 남길 수 있게 한다. 값은 `franchise_locations.data.siteCondition` 같은 구조화 필드로 저장한다.
- 임대인 정보는 `임대인명`, `임대인 연락처`, `임대인정보 및 성향`을 내부 검토용으로 둔다. 임대인 연락처와 성향은 외부 브리핑/브랜드 전송에는 기본 노출하지 않는다.
- 점포구입비는 `보증금 + 권리금` 합계로 계산한다. 출점 후보지 데이터에 `deposit`, `premium`, `acquisitionCostMemo`를 저장하고, 목록에서는 `점포구입비`와 메모 요약을 함께 볼 수 있게 한다.
- 임차조건은 `월세`, `관리비`를 기본 금액으로 두고, `임차조건 메모`를 별도 입력으로 둔다. 출점 후보지 데이터에 `monthlyRent`, `maintenanceFee`, `leaseConditionMemo`를 저장한다.
- 종합메모는 후보지의 현장 판단을 한 번에 읽을 수 있는 상단 메모로 둔다. 현재 `memo`는 유지하되, 점포구입비 메모와 임차조건 메모를 합쳐 보지 않아도 되게 별도 요약 영역을 둔다.
- 출점 후보지 목록 기본 컬럼은 `중요도`, `개발상태`, `후보지명`, `주소`, `실평수`, `점포구입비`, `보증금`, `권리금`, `월세`, `관리비`, `물리조건`, `임대인 성향`, `종합메모`, `담당자`, `등록일` 중심으로 재구성한다.
- 필터는 요구사항 문서 기준으로 `지역`, `점포구입비`, `보증금`, `권리금`, `월세`, `관리비`, `중요도`, `진행상태`, `개발상태`를 우선한다. 출점 후보지 화면 안에서 저장/재사용되는 필터를 1차 목표로 두고, 회사 공용 필터 저장은 2차 범위로 둔다.
