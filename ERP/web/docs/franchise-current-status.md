...
- 신규 SQL: 공통 일정/결재 MVP용 `supabase_franchise_approval_calendar_migration.sql`을 추가했다. 이 SQL은 기존 `schedules` 확장과 `approval_templates`, `approval_documents`, `approval_document_events` 테이블, source 중복 방지 인덱스, 회사 범위 RLS를 포함한다. 대상 Supabase 환경에는 사용자가 직접 SQL을 등록해야 한다. **SQL 등록 필요**.
