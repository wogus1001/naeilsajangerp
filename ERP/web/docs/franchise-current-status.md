...
## 2026-08-04 모객 DB 계약 가맹점 연결·후보지 경계 보강

- 계약완료 상세의 `가맹점 정보`에서 새 가맹점 수기 생성뿐 아니라 회사의 기존 가맹점 목록을 선택해 계약 점주와 연결할 수 있다. 이미 다른 계약 점주와 연결된 가맹점과 후보지 항목은 서버에서도 연결을 거부하며, 중복 연결 경합은 충돌 안내로 처리한다.
- 새로 연결할 출점 후보지는 `예정점`, `검토중`, `오픈준비`만 노출한다. `직영점`, `가맹점`, `운영중`, `휴점`, `폐점`은 신규 후보에서 제외하되, 후보지로 연결된 뒤 운영 가맹점으로 전환된 항목은 기존 연결 기록에서 계속 확인할 수 있다.
- 가맹 희망자 등록·수정 화면의 불필요한 메모 입력을 제거하고, 유입경로 기본 항목 `기타`를 회사가 이름 변경·사용 중지·재사용할 수 있는 일반 항목으로 전환했다. Meta Lead Ads, 고객DB, 명함DB, 가맹 희망자 등록, 예비 창업자 등록은 계속 고정 항목으로 보호한다.
- 기존에 회사별 유입경로 migration을 적용한 DB에는 `supabase_franchise_lead_source_other_editable_migration.sql`을 추가 적용해야 한다. 최신 기본 migration은 신규 DB에서 `기타`를 수정 가능 항목으로 생성한다. **SQL 등록 필요**.
- 관련 회귀 테스트 35건, TypeScript, ESLint, production build 113개 페이지와 `git diff --check`를 통과했다. 인증된 로컬 화면에서 계약완료 상세와 기존 연결 가맹점 표시를 확인했으며 Next.js 오류 오버레이와 화면 깨짐은 없었다.

## 2026-08-03 데모 실화면 동기화·모객 DB 단계 운영 보강

- 공개 `/demo`의 대시보드, 모객 DB, 후보자 상세 팝업, 물건지 지도, 가맹 운영 화면은 운영용 컴포넌트를 그대로 조합하고 샘플 전용 상태·데이터 source만 주입한다. 데모 전용 API/외부 이동 차단은 유지해 실제 회사 데이터나 보호 API를 호출하지 않는다.
- 대시보드 헤더, 알림, 프로필 메뉴, 중앙 알럿·확인창, 주소·브랜드 검색, 후보자 상세의 문서·계약·오픈 준비 영역을 prop/runtime 경계로 분리했다. 운영 라우트는 주입값이 없을 때 기존 live API와 UI를 그대로 사용한다.
- 모객 DB에서 등록 버튼은 현재 선택한 `1차 유입 DB` 또는 `가맹 희망자` 단계를 저장한다. 목록에는 `최근 연락 일자`, `최근 이력 내용`을 표시하고 상담 유형에 `부재`를 추가했다. 가맹 희망자 다중 선택 툴바에는 `연락일 적용` 옆 `1차 유입 DB로 이동`을 추가했으며 확인 후 일괄 이동하고 원본 DB 탭으로 전환한다.
- 일반 후보자 수정은 단계 값을 잘못 섞어 보내도 후보자 상태를 보존하고, 명시적인 일괄 복귀 요청만 `raw_intake`로 변경한다. 이 이동에는 신규 SQL이 없고 기존 `franchise_leads.data.leadStage`를 사용한다.
- 변경·신규 회귀 테스트 102건, TypeScript, 전체 ESLint, production build 113개 페이지와 `git diff --check`를 통과했다. 인증된 로컬 브라우저에서 후보자 1건을 `1차 유입 DB`로 이동해 건수 `1/3 -> 2/2`와 자동 탭 전환을 확인한 뒤 원복했고, console error 0건과 독립 기능·한국어 UI 리뷰 `PASS`를 확인했다.
- `npm run qa:demo-ui` 자동 smoke를 추가했다. 실행 셸에 `DEMO_ACCESS_ID`, `DEMO_ACCESS_PASSWORD`가 없으면 자격정보를 요청하며 중단하므로 값은 저장소에 기록하지 않는다.

## 2026-07-30 모객 DB 유입경로 관리·필터 정리

- 가맹 희망자 등록·수정 화면의 `유입경로`에서 회사별 항목을 관리한다. 네이버폼, 랜딩페이지, 박람회, 소개, 광고, 전화문의, 기타와 회사가 추가한 항목은 이름 변경·사용 중지·재사용이 가능하다.
- Meta Lead Ads, 고객DB, 명함DB, 가맹 희망자 등록, 예비 창업자 등록은 자동 수집과 다른 DB 연결의 안정 식별값이므로 UI, API, DB trigger에서 수정·사용 중지를 막는다. 수정 가능한 항목도 실제 저장 코드는 유지하고 표시 이름만 바꿔 기존 리드·필터·내보내기 호환성을 보존한다.
...
- 각 회사가 연결한 Meta 신청 양식의 실제 질문을 이름, 연락처, 희망 지역, 예산, 관심 브랜드, 메모 등 모객 DB 고정 항목에 직접 연결한다. 자동 추천 후 운영자가 질문별 연결을 수정하거나 `연결 안 함`을 선택할 수 있고, 이름·연락처·기본 담당자가 준비된 양식만 자동 수집을 켤 수 있다.
- 자동 연결은 화면의 임시 변경으로만 적용하고 `연결 저장`을 눌러야 영속화한다. 저장 전 매핑은 담당자 변경, 항목 새로고침, 상태 새로고침에도 유지하며 원래 연결로 되돌리면 변경 상태도 해제한다. Meta 재연결·양식 재발견은 운영자가 저장한 수집 여부, 담당자, 매핑을 덮어쓰지 않는다.
- Webhook은 Page와 Form 소유 회사를 함께 확인하고 여러 회사에 걸쳐 모호하면 저장하지 않는다. Page token은 URL이 아닌 Bearer header로 전송하고 10초 timeout을 적용했다. 클라이언트에는 정규화한 질문만 반환하며 공급자 원본과 과도한 매핑 payload는 노출·저장하지 않는다.
- 기능 커밋은 `36fd18c feat(franchise): Meta 신청 항목 매핑 보강`이고 작업 버튼·정렬 후속 커밋은 `ed3240a fix(franchise): Meta 연동 작업 버튼 정리`다. Form별 중복 `신청 내역 가져오기`를 제거하고 상단 단일 액션을 `전체 신청 가져오기`로 명시했으며, 상태 확인과 질문 갱신은 각각 `연결 확인`, `질문 다시 불러오기`로 구분했다.
- Meta 관련 회귀 43건과 후속 집중 회귀 22건, TypeScript, 전체 ESLint, production build 113개 페이지, `git diff --check`를 통과했다. 로컬 Codex 브라우저 1280px에서 최신 버튼 구성과 패널 렌더, console error 0건을 확인했고 반응형 CSS 게이트에서 1180px 이하 제목·액션 적층과 720px 이하 한국어 도움말 줄바꿈을 확인했다.
- 실제 연결 회사에서 질문 매핑 저장·복원 후 새 테스트 신청이 이름·연락처·희망 지역·예산·관심 브랜드·메모에 들어오는지 확인하는 live QA는 Dev 배포 후 진행한다. 기존에 가져온 신청은 매핑 변경만으로 재변환되지 않으므로 새 테스트 신청을 생성해 검증한다. 신규 SQL과 공개 `/landing`·`/demo` 변경은 없다.
...
- 모객 DB 표의 헤더·본문·체크박스 셀 좌우 여백을 12px로 통일했다. 기존 컬럼 폭과 표 내부 가로 스크롤은 유지하고, 모바일의 페이지 크기 선택과 한국어 설명 문구가 자연스럽게 배치되도록 보정했다.
- 기간 저장·회귀 테스트, TypeScript, ESLint, production build, `git diff --check`와 1600px·1280px·390px 브라우저 QA를 통과했다. 신규 SQL과 공개 `/landing`·`/demo` 변경은 없다.

## 2026-07-23 로그인·가입·모객 DB QA 안정화

- 회사 찾기 결과에서 현재 지원하지 않는 대표자 정보를 제거했고, 가입 중 일시적인 JWT key 오류는 제한 재시도로 복구한다. 직원 승인 대기 목록에서는 사용자에게 필요 없는 내부 UUID를 숨긴다.
- 모바일 로그인의 `회사 찾기` 버튼이 카드 밖으로 잘리지 않도록 배치를 보정했다.
- 모객 DB 표는 8px 단위 컬럼 폭으로 정리하고 희망지역·예산을 각각 120px로 축소했다. 넓은 화면의 남는 공간은 표 오른쪽이 흡수하고, 좁은 화면은 표 내부 가로 스크롤을 유지한다. 컬럼 선택 메뉴는 카드에 잘리지 않고 자체 스크롤로 전체 항목을 표시한다.
- 관련 회귀 테스트와 TypeScript, 대상 ESLint, 화면 QA를 통과했다. 신규 SQL과 공개 데모 변경은 없다.

## 2026-07-21 입점 요청 사진 업로드 긴급 보정

- 운영에서 큰 사진 첨부 시 Vercel 함수 본문 제한으로 `/api/upload`가 `413 Request Entity Too Large`를 반환했고, 입점 요청 DB가 먼저 저장된 뒤 사진 업로드가 실패해 목록에는 등록 건이 보이지만 첨부 사진 URL은 없는 상태가 발생했다.
...
- 점주 문의 SLA 자동화 3단계 1차는 최신 SQL 적용과 적용 DB 재검증을 거쳐 PR #32로 `main`에 승격했다. 운영 deployment `dpl_BTeDA4c4CSFLueekD9r4ZQwo2ALy` READY와 두 운영 도메인의 주요 경로 200 응답을 확인했다. **SQL 등록 완료 확인**.
...
- 관리자에게만 `삭제 목록` 탭을 노출하며 유형, 제목, 회사, 삭제자, 삭제일, 요약을 확인한다. `상세 확인`은 삭제 시점 스냅샷을 기존 입점 요청·예비 창업자 확인 화면과 같은 읽기 전용 폼으로 복원해 임대 조건, 면적, 지원 조건, 상담 내용, 후속 조치, 첨부 등 전체 등록 내용을 보여준다. 관리자는 중앙 위험 확인 알럿을 거쳐 삭제 이력 row를 복구 불가능하게 `완전삭제`할 수 있으며, 일반 계정은 UI와 API 모두 접근할 수 없다. 과거 URL-only 첨부도 현재 Supabase origin과 삭제 원본 ID 경로가 모두 일치할 때만 열며, 삭제 이력을 저장할 수 없으면 원본도 삭제하지 않는 fail-closed 정책을 사용한다.
...
- 최신 `supabase_franchise_work_intake_deleted_records_migration.sql`은 원본 row 잠금, 서버측 전체 스냅샷, 중복 방지, 삭제 건수 검증과 `properties.id(text)` 타입 보정을 포함한다. 사용자 확인과 운영 DELETE 200 로그 기준 2026-07-20 운영 DB 적용을 완료했다. 완전삭제 API는 기존 삭제 이력 테이블을 사용하므로 신규 SQL은 없다. **SQL 등록 완료 확인**.
...
- 순차 개발 현재 단계: 전자결재 1단계와 가맹운영 일정·알림 2단계는 코드·SQL·dev/main·production 승격을 마쳐 `완료`다. 점주 포털과 본사 업무 자동화 3단계는 문의 SLA 1차를 운영에 반영했고, 남은 리마인드·문서 수령·자료 라이브러리·정산 증빙을 하나의 통합 개발 범위로 진행한다.
- 3단계 통합 범위: 체크리스트 미완료 리마인드와 본사 집계, 공지·공문·시정요청·계약 문서 수령 확인, 교육자료·운영 매뉴얼의 버전·열람 관리, 정산·영수증·증빙 제출과 비공개 파일 관리를 한 데이터 계약과 권한 모델로 묶는다. 구현은 네 개의 내부 검증 게이트로 나누지만 3단계 완료 전까지 별도 제품 단계로 분리하지 않는다.
- 3단계 통합 구현 상태: 게이트 A 리마인드·수령 확인, 게이트 B 자료 라이브러리, 게이트 C 정산·증빙의 공통 데이터 모델·RLS·Storage 계약과 본사/점주 API·UI 구현을 완료했다. 반복 migration의 과거 이력 보존, 최초 발송 대상을 그대로 반환하는 리마인드 재시도, 보관 자료의 기존 리마인드 확인, 초안을 포함한 모든 자료 변경의 버전 잠금과 첨부 운영점 경계, 정산 요청 멱등성·원문 동시성 토큰, 파일 SHA-256 재시도 검증, 중단 업로드 정리 outbox, 정산 검토의 가맹운영 일정 연동까지 리뷰 보정을 반영했다. 정산 일정 복구는 전체 제출 건을 페이지 순회하되 누락·상태 불일치 건만 다시 동기화해 이미 정상 처리된 알림을 반복 갱신하지 않는다. 자동 검증과 비인증 경계 QA는 통과했지만 적용 DB와 private Storage를 사용하는 실계정 게이트 D는 아직 실행하지 않았으므로 3단계 제품 완료로 판정하지 않는다.
- 3단계 통합 SQL: `supabase_franchise_owner_phase3_migration.sql`을 dev DB에 먼저 적용하고 schema cache와 `franchise-owner-private` bucket 정책을 확인해야 한다. 이후 본사 관리자·일반 직원·점주 계정으로 회사/운영점 교차 접근 차단, 자료 버전별 열람·수령, 정산 증빙 제출·반려·재제출, signed URL 다운로드, 파일 삭제 outbox를 검증한다. **SQL 등록 필요**.
- 플랫폼 안정화 4단계 1차: 관리자 `/admin/operations` 운영센터에서 일정 동기화, 점주 포털 파일 정리, 알림톡 실패·차단을 한 화면으로 조회한다. 일정·파일 작업은 확인 후 재처리할 수 있고 요청 ID와 처리 전후 상태를 감사 이력으로 남긴다. 알림톡은 불완전한 과거 로그의 오발송을 막기 위해 확인 전용이다. 사용자 확인 기준 `supabase_platform_operations_phase4_migration.sql` 적용을 완료했으며, 실패 샘플 재처리·worker 완료·감사 이력 생성의 적용 DB QA가 남아 있다. **SQL 등록 완료 확인**.
...
- 2단계 내구성 보강: 프로필 보안 SQL과 `supabase_franchise_schedule_durable_sync_migration.sql`, `supabase_franchise_schedule_durable_sync_review_fix_migration.sql`은 사용자 확인 기준 적용 완료했다. 원천 일정과 수신자 알림을 PostgreSQL 트랜잭션과 advisory lock 안에서 함께 갱신하고, 일시 실패는 `franchise_schedule_sync_jobs`에 최신 payload로 보관해 예약 실행에서 재시도한다. 리뷰 보완에서는 큐 선기록과 UUID lease 검증으로 오래된 worker의 덮어쓰기를 막고, 재시도 직전 수신자 자격을 다시 확인하며, 업체 계약 담당자와 SV 방문 삭제의 회사 범위를 보강했다. 후속 코드 보정은 큐 자체를 저장하지 못한 실패를 호출자에게 전달하고, 한 재처리 건의 프로필 오류가 다음 작업을 막지 않게 격리하며, 보고서 저장 후 시정요청 후처리 실패는 성공 응답의 지연 경고로 반환해 중복 보고서 생성을 막는다. **SQL 등록 완료 확인**.
...
## 2026-07-22 정보공개서 Gmail OAuth 연결 보정
