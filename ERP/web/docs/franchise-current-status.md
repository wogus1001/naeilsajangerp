...
- 최신 운영 배포 기준 커밋: `6bc9455 개선 슈퍼바이징 AI 보고서 출력 정리` (`dpl_4keMEHZv2Qd4H2MwAY6QNrYiqmnD`, `https://www.fcerp.co.kr` READY). 슈퍼바이징 AI 점검 보고서의 출처 접두어 반복 제거, PDF/인쇄 조치 필요 항목 요약화, 전체 점검 내역 상세 분리를 포함한다.
...
- 점주 포털 1차: 기존 본사 `/login`과 `profiles` 직원 권한 체계에 섞지 않고 `/owner/login`, `/owner/dashboard`를 별도 공개 라우트로 추가했다. 본사는 `가맹 운영 > 점주 연동`에서 운영점별 점주 계정을 발급/중지/활성화/비밀번호 재발급하고, 점주는 전용 HttpOnly 세션으로 자기 운영점 하나만 접근한다. 점주 화면은 내 매장 기본 정보 제출, 공지 읽음 처리, 오픈 체크리스트 완료 요청, 시설/고장 문의, 최근 제출 이력을 제공한다. 본사 큐는 점주 제출 건을 승인/반려/보관 처리하며, 오픈 체크리스트 완료 요청은 본사 승인 후에만 연결된 오픈 준비 프로젝트 task에 반영한다. 신규 `supabase_franchise_owner_portal_migration.sql` 적용이 필요하다. **SQL 등록 필요**.
- 배포 상태: 직원 관리/개인정보 수정 보강, 개인정보 저장 핫픽스, 공개 사업자정보 푸터, 진행현황 권한/삭제, 출점 검토 리포트 버전 이력·상권분석 근거, legacy requester/admin fallback 제거 보안 하드닝, 모객 DB/고객/명함/점포개발 화면 세션 header 보강, 출점 검토 리포트 Kakao 상권 지도, 업체 계약함 등록/수정 페이지 분리, Vercel `naeilsajang` 프로젝트 Node.js 24.x 정렬, 어드민 관리 메뉴 순서 변경, 정보공개서 확인 안내 알림톡 연동, 슈퍼바이징 AI 점검 보고서 출력 정리, 슈퍼바이징 승인·시정요청 세션 분리는 운영 배포 완료 상태다. 최신 배포는 `dpl_9F5AmwUrYQnumoUDYsSHx4ApqtL1`이며, Vercel inspect에서 `name=naeilsajang`, `target=production`, `status=Ready`, aliases `https://www.fcerp.co.kr`, `https://fcerp.co.kr`를 확인했다.
...
- 신규 SQL: 점주 포털용 `supabase_franchise_owner_portal_migration.sql`을 추가했다. 이 SQL은 점주 계정, 점주 전용 세션, 공지/읽음 기록, 제출 이력, 업로드 파일 메타 테이블과 RLS 정책을 포함한다. 대상 Supabase 환경에는 사용자가 직접 SQL을 등록해야 한다. **SQL 등록 필요**.
...
- 점주 포털 SQL 적용 후 실계정으로 `가맹 운영 > 점주 연동`에서 점주 계정 생성, 임시 비밀번호 안내 복사, 계정 중지/활성화/재발급, `/owner/login` 점주 로그인, 매장 정보 제출, 공지 읽음, 오픈 체크리스트 완료 요청, 시설 문의 등록, 본사 제출 큐 승인/반려 persistence를 확인한다.
