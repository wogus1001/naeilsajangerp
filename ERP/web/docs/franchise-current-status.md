...
- 순차 개발 현재 단계: 전자결재 1단계는 문서함 검색·필터 실계정 QA를 남긴 `검증 중`이다. 사용자 승인 예외로 먼저 진행한 가맹운영 일정·알림 2단계는 원천 연결, 상태·담당자 수명주기, 저장소 경계, 자동/mock-session QA와 코드 보안 보정까지 완료했다. 운영 마감은 신규 프로필 보안 SQL 적용 확인 전까지 `코드 완료·SQL 적용 대기`로 관리한다. 1단계 잔여 QA는 별도 상태로 유지하며 dev/production 승격 전 필수 확인한다.
...
- 2단계 후속 운영 고도화: `supabase_franchise_source_schedule_profile_security_migration.sql` 적용 확인이 운영 마감 선행 조건이다. 이후 기존 SV 파일럿 `schedules` 행의 이관·종료, 원천 저장 후 일정 동기화 실패를 영속적으로 재처리할 outbox/reconciliation, 원천 변경 없이 KST 자정에 지연 상태를 재평가하는 실행기, 적용 DB 실계정 원천 회귀가 남아 있다. 남은 브라우저 기본 팝업 제거는 별도 UI 운영 고도화이며, 1단계 문서함 실계정 QA는 dev/production 승격 전 별도 게이트다.
...
- 신규 SQL: 가맹운영 원천 일정 upsert 보강용 `supabase_franchise_source_schedule_upsert_migration.sql`은 사용자 확인 기준 대상 DB 적용 완료다. 이어서 비활성 계정·협력업체 담당자와 잘못된 관리자 역할을 RPC에서 거부하는 `supabase_franchise_source_schedule_profile_security_migration.sql`을 적용해야 한다. 적용 순서는 `upsert` 보강 SQL 다음 `profile security` SQL이며, 적용 후 업체 계약·정보공개서와 2차 원천의 실계정 동기화 QA를 진행한다. **프로필 보안 SQL 등록 필요**.
