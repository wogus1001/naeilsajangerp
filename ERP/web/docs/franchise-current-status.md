...
- 3단계 SQL: `supabase_franchise_owner_submission_sla_migration.sql`을 `supabase_franchise_schedule_durable_sync_review_fix_migration.sql` 후에 적용해 일정 지연 재판정을 정확한 `due_at` 시각 기준으로 보정해야 한다. **SQL 등록 필요**.
