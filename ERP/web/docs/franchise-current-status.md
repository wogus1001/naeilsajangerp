...
- 가맹 희망자 등록·수정 화면의 `유입경로`에서 회사별 항목을 관리한다. 네이버폼, 랜딩페이지, 박람회, 소개, 광고, 전화문의와 회사가 추가한 항목은 이름 변경·사용 중지·재사용이 가능하다.
- Meta Lead Ads, 고객DB, 명함DB, 가맹 희망자 등록, 예비 창업자 등록, 기타는 자동 수집과 다른 DB 연결의 안정 식별값이므로 UI, API, DB trigger에서 수정·사용 중지를 막는다. 수정 가능한 항목도 실제 저장 코드는 유지하고 표시 이름만 바꿔 기존 리드·필터·내보내기 호환성을 보존한다.
