...
- 최신 운영 배포 기준 커밋: `86faac9 보정 점주포털 체크리스트 릴리즈 연결` (`dpl_4WrjSSxde5Ggxq8jMjpFsYmbpTot`, `https://www.fcerp.co.kr` READY). 점주 포털 회사별 단축 로그인 링크, 점주 로그인 회사명 입력 미노출, 점주 운영 체크리스트 전체/복수 운영점 저장 흐름을 포함한다.
...
- 점주 포털 SQL 적용 후 실계정으로 `가맹 운영 > 점주 소통`에서 공지/공문 발행과 읽음 현황, 운영 체크리스트 전체/복수 운영점 발송, 체크리스트별 완료/미완료 운영점 상세, 점주 계정 생성/중지/활성화/재발급, `/owner/login/{companyId}` 전용 링크 로그인, 매장 정보 제출, 공지 읽음, 운영 체크리스트 완료 요청, 시설 문의 등록, 본사 제출 처리/보관 persistence를 확인한다.
