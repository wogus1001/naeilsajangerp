...
- 점주 포털 SQL 적용 후 실계정으로 `가맹 운영 > 점주 소통`에서 공지/공문 발행과 읽음 현황, 운영 체크리스트 전체/개별 저장, 점주 계정 생성/중지/활성화/재발급, `/owner/login` 회사명 기반 점주 로그인, 매장 정보 제출, 공지 읽음, 운영 체크리스트 완료 요청, 시설 문의 등록, 본사 제출 처리/보관 persistence를 확인한다.
