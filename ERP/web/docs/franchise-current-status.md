...
- 검색·삭제 이력 기반 기능은 `main` `82e3d88`까지 반영됐고, 전체 삭제 상세와 메뉴 단일 활성 보정은 `dev` `7c1a143`에서 QA를 마쳤다. 운영 승격 후보는 legacy URL-only 첨부의 교차 원본 연결 차단까지 포함한 `origin/main` 기반 release `c85bf36`이며 preview `dpl_5b7MS7Cikc1k4odMVWDs6Qui1eE6`가 `Ready`다. main/production 반영은 이 release 검증 이후 진행하며, 작성자/일반 직원/팀장/관리자/협력업체 실계정 권한과 실제 삭제 이력 persistence는 운영 live QA로 남긴다.
...
- 2단계 원천 일정 1차: 업체 계약 만료일과 정보공개서 계약 가능일을 가맹 운영 일정으로 동기화한다. 프랜차이즈 원천 일정은 `franchise_schedules`와 `/dashboard/franchise-operations/schedule`만 사용하며, 점포개발 업무의 `schedules` 또는 `/schedule`에는 저장하거나 표시하지 않는다. 계약 ID와 후보자 ID를 원천 키로 사용해 반복 동기화 시 한 건만 유지하고, 계약 갱신·종료와 정보공개서 계약 가능 상태를 일정 상태에 반영한다. 전용 upsert RPC는 서비스 역할만 실행할 수 있고 회사가 다른 담당자 연결을 거부하며 최초 완료 시각을 보존하도록 `supabase_franchise_source_schedule_upsert_migration.sql`에서 보강한다. **SQL 등록 필요**.
- 다음 개발 묶음: 1단계 문서함 검색·필터 실계정 QA를 배포 승격 전에 마감하고, 2-2에서 남은 프랜차이즈 기본 팝업 제거와 SV·오픈 준비·점주 문의·체크리스트의 원천 일정을 같은 가맹 운영 전용 저장소에 순서대로 연결한다. 기존 스키마로 충족할 수 없는 경우에만 migration을 작성한다.
...
- 최신 QA: 다이얼로그 큐, 알림 링크 보안, 가맹 운영 원천 일정 분리를 포함한 전체 자동 테스트 743건, `tsc`, `lint`, `build`, `git diff --check`가 통과했다. 전자결재 PDF 실다운로드·A4 한글 렌더링, 전자결재, 가맹운영 일정, 업무 접수 주요 화면을 확인했고 가맹운영 일정 최신 브라우저 QA의 console error는 0건이었다.
...
- 공통 일정/결재 MVP: `/schedule`의 기존 점포개발 업무용 달력/작업내역/일정내역 화면은 `점포개발 일정` 탭에 그대로 두고, 새 `전사 업무·결재` 탭에 `오늘 처리`, `승인 대기`, `지연 업무`, `이번주 일정` KPI와 업무 큐를 추가했다. 신규 `/api/franchise-approvals/templates`, `/api/franchise-approvals/documents`, `/api/franchise-approvals/actions`는 회사 범위 결재 양식/문서/상태 전이를 처리한다. `schedules`는 `source_type/source_id`, 담당자/관리자, 마감/완료 시각, metadata를 갖는 source 기반 업무 허브로 확장한다. 파일럿으로 SV 방문 일정은 `supervision-visit`, SV 점검 보고서 제출은 `supervision-report` 결재 문서와 `approval-document` 승인 대기 일정에 연결한다. 보고서 제출 시 관리자 인앱 알림과 승인 대기 일정이 생성되고, 승인/반려 시 작성자 인앱 알림과 승인 대기 일정 완료 처리가 진행된다. `supabase_franchise_approval_calendar_migration.sql`은 2026-07-10 운영 DB 적용 완료다. **SQL 등록 완료 확인**.
...
- 공통 일정/결재 SQL 적용 후 실계정으로 `/schedule`의 `점포개발 일정` 탭이 기존 달력/작업내역 그대로 보이는지, `전사 업무·결재` 탭에서 오늘 처리/승인 대기/지연 업무/이번주 일정 큐가 노출되는지, SV 방문 일정이 `SV 방문` source badge로 표시되는지, SV 점검 보고서 제출 후 관리자 승인 대기 일정과 인앱 알림이 중복 없이 생성되는지, 승인/반려 후 작성자 알림과 승인 대기 일정 완료 처리가 되는지 확인한다.
...
- 순차 개발 로드맵의 1단계 `전자결재 운영 완성`을 개발 중으로 전환했다.
