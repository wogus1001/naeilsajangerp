...
- 고객 목록에 가장 최근 고객작업내역의 `작업 날짜`, `작업 내역`을 같은 기록 기준으로 표시하고 정렬할 수 있다. 작업 내역은 표에서 말줄임으로 보여주고 전체 내용은 도움말로 확인한다.
- `표시 컬럼`에서 19개 업무 컬럼을 켜고 끌 수 있으며, 드래그 또는 키보드 이동으로 순서를 바꾼다. 선택 결과는 브라우저에 저장해 재방문 시 복원한다.
- 열 너비 조절 위치에는 항상 얇은 구분선을 표시한다. 관심고객 별표는 `중요` 열로 분리해 선택 체크박스 바로 뒤에 고정하고 버튼 중앙 정렬과 키보드 포커스를 보강했다.
- 같은 회사의 담당자 미지정 고객은 일반 담당자도 작업내역을 기록할 수 있다. 다른 담당자에게 배정된 고객 보호는 유지하며, 저장 실패 시 서버의 한국어 오류를 표시하고 화면에 임시 추가한 작업내역을 원복한다. 사용자가 확인한 production의 기존 정상 저장 흐름은 변경하지 않는다.
- 기능 커밋은 `e281a3e feat(customers): 고객 작업내역 컬럼 설정 보강`이다. 집중 회귀 15건, TypeScript, ESLint, production build 113개 페이지, `git diff --check`와 인증된 로컬 브라우저 QA를 통과했다.
- 신규 SQL과 환경변수 변경은 없다. `/landing`, `/demo`, 프랜차이즈 화면에는 영향이 없다.

## 2026-08-06 프랜차이즈 데모 업무 스토리·실화면 일치 마감

- `/demo` 첫 진입은 전체 흐름과 가맹 영업, 출점 개발, 오픈 준비, 본부 관리자 시나리오를 선택하는 방식이다. 각 시나리오는 실제 프랜차이즈 메뉴와 운영 컴포넌트를 사용해 광고 유입부터 상담, 정보공개서, 후보지/물건 기록, 계약 준비, 가맹 운영, 점주 계정과 점주 홈페이지까지 이어진다.
- 투어는 목적지 화면·탭·상세 패널을 먼저 연 뒤 대상 위치를 다시 계산한다. 시나리오 전환 시 단계가 잘못 초기화되거나 딤드 강조가 화면과 어긋나는 문제, 상세 하단이 잘리는 문제를 보정했다.
- 데모 내부의 실제 API·외부 발송은 차단하며 fixture와 메모리 상태만 변경한다. 운영 화면은 기존 live fetch와 navigation 계약을 유지한다.
- 모객 DB 중복 연락처 저장 실패는 `같은 연락처로 등록된 가맹 희망자가 이미 있습니다.`로 안내한다.
- 전체 테스트 1,103건, 데모 집중 회귀 32건, 중복 연락처 회귀 6건, TypeScript, ESLint, production build, diff check와 로컬 브라우저 QA를 통과했다. 독립 코드·목표·보안·QA·최종 게이트 리뷰도 모두 통과했다.
- 배포 단위는 `4253ecb`부터 `348b518`까지 5개 커밋이다. 신규 SQL·환경변수·`/landing` 변경은 없다.
