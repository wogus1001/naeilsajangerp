...
- 배포 상태: 직원 관리/개인정보 수정 보강, 개인정보 저장 핫픽스, 진행현황 작성자 표시까지 운영 배포 완료 상태다. `naeilsajang` production inspect에서 `www.fcerp.co.kr`/`fcerp.co.kr` alias와 Ready 상태를 확인했다. 진행현황 권한/삭제 및 프리셋 UI 정리, 공개 사업자정보 푸터, 진행현황 권한 안내 문구 정리는 이번 운영 배포 대상으로 검증 완료했다.
