...
## 2026-07-10 프랜차이즈 전용 일정관리 분리 QA

- 범위: 점포개발 업무의 `/schedule`과 별개로 `가맹 운영 > 일정관리`(`/dashboard/franchise-operations/schedule`)를 추가했다. 새 화면은 `franchise_schedules` 전용 API만 사용하고 기존 점포개발 `schedules` 테이블과 연동하지 않는다.
- API/데이터: 신규 `/api/franchise-schedules`는 회사 범위 조회, 수동 일정 생성/수정/삭제/완료를 제공한다. `source_type/source_id`가 있는 workflow 일정은 원천 업무에서만 갱신하며 공개 API에서 수정/삭제/완료를 403으로 차단한다. SQL 미적용 시 `supabase_franchise_schedule_prepare_migration.sql` 적용 안내를 반환한다.
- 화면: 프랜차이즈 전용 월간 캘린더, 선택 날짜 목록, 오늘 처리/이번주/승인 대기/지연 KPI, 업무 큐 필터와 페이지네이션, 수동 일정 등록 모달을 추가했다. 가맹 운영 권한을 그대로 사용하므로 기존 회사 메뉴 설정이 있어도 새 feature flag 때문에 숨겨지지 않는다.
- 신규 SQL: `supabase_franchise_schedule_prepare_migration.sql`, `supabase_franchise_schedule_cutover_migration.sql`, `supabase_franchise_schedule_cutover_rollback.sql`을 추가했다. 대상 Supabase 환경에는 사용자가 직접 SQL을 등록해야 한다. **SQL 등록 필요**.
- 검증: `npx tsx --test src/lib/franchise-schedules.test.mts src/lib/franchise-schedule-store.test.mts src/lib/franchise-schedule-migration.test.mts src/app/api/schedules/route.test.mts src/app/api/dashboard/route.test.mts src/app/api/franchise-schedules/route.test.mts 'src/app/(main)/schedule/store-schedule-model.test.mts' src/components/franchise/schedules/franchiseScheduleViewModel.test.mts src/lib/company-menu-features.test.mts` 50건 통과. `npx tsc --noEmit --pretty false --incremental false`, `npm run lint -- --quiet`, `npm run build`, `git diff --check`, `node scripts/qa/franchise-schedule-scope-scan.mjs origin/dev` 통과. `node scripts/qa/franchise-schedule-visual-qa.mjs`로 mock 세션에서 `/dashboard/franchise-operations/schedule` desktop/mobile 렌더링, `오늘 처리` 필터 클릭, console error 0건을 확인했다. DB migration runtime QA는 `TEST_DATABASE_URL`이 없어 `PENDING_EXTERNAL`로 남겼다.
- 남은 live QA: SQL 적용 후 실계정으로 가맹 운영 일정 탭 진입, 수동 일정 등록/완료/삭제 persistence, source 일정 읽기 전용 표시, 기존 점포개발 `/schedule` 데이터와 미연동을 확인한다.
