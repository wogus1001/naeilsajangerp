...
- 모객 DB 업무 큐에서 `전체 업무`, `연락 지연`, `오늘 연락`, `무응답` 필터별 건수가 실제 카드 목록과 맞는지 확인
- 후보자 상세에서 다음 액션, 상담 결과, 이탈 사유, 자금/지역/브랜드 적합도 저장 후 새로고침해도 값이 유지되는지 확인
- `연락 완료` 처리 시 다음 연락일이 비워지고 상담 결과가 `연락 성공`으로 저장되는지 확인
- 후보자 상세의 후보지 연결 패널에서 `출점 후보지`와 `외부 상가 DB`를 검색/연결/상태 변경/메모 저장/삭제할 수 있는지 확인
- 같은 출점 후보지 또는 외부 상가를 여러 후보자에게 중복 연결할 수 있는지 확인
...
- 결과 표가 주소 중심으로 표시되고, 저장일/별표/가격/세부/반응/원문 링크가 깨지지 않는지 확인
