...
### 2026-08-06 고객 목록 작업내역·컬럼 설정 QA

- 고객 목록에 고객작업내역의 최신 기록을 기준으로 `작업 날짜`, `작업 내역`을 함께 노출했다. 두 열은 같은 최신 기록을 사용하고, 각각 정렬할 수 있으며 긴 작업 내역은 표에서 말줄임하고 전체 내용은 도움말로 확인한다.
- `표시 컬럼` 메뉴는 19개 업무 열의 표시 여부를 관리한다. 고객명은 필수 열로 유지하고, 나머지는 드래그 또는 키보드 위·아래 이동으로 순서를 바꿀 수 있다. 설정은 `customerListColumnPreferencesV1`에 저장해 같은 브라우저에서 복원한다.
- 열 너비 조절은 기존 4px 조작 영역을 유지하면서 항상 보이는 1px 구분선을 추가했고, hover 시 2px 파란선으로 바뀐다. 관심고객 별표는 빈 머리글 대신 `중요`로 표시하고 `선택 -> 중요 -> No -> 고객명` 순서에 고정했다.
- 로컬에서 담당자 미지정 고객의 작업내역 저장이 403으로 실패하던 경계는 같은 회사 사용자에 한해 허용했다. 다른 담당자에게 배정된 고객은 기존처럼 담당자 본인·팀 관리자만 수정할 수 있다. 저장 실패 응답은 실제 한국어 오류를 보여주고, 낙관적으로 추가한 작업내역은 원래 상태로 되돌린다. 사용자는 production에서는 기존 저장이 정상임을 확인했다.
- 자동 검증: `customerListTableConfig.test.mts`, `customer-update-access.test.mts` 집중 회귀 15건, `npx tsc --noEmit --pretty false --incremental false`, `npm run lint -- --quiet`, `npm run build`, `git diff --check`를 통과했다. build는 113개 페이지를 생성했고 기존 workspace root, `baseline-browser-mapping`, Browserslist 경고만 남았다.
- 브라우저 QA: 인증된 로컬 `/customers`에서 작업 대상이 비어 있어도 작업내역 저장이 성공하는 흐름, 목록의 작업 날짜·내역 표시, 컬럼 표시/복원, 드래그 가능 속성, `중요 -> No` 머리글 순서와 항상 보이는 열 구분선을 확인했다. QA용 고객은 삭제했고 Next.js 오류 오버레이와 console error는 없었다. 기존 Supabase GoTrueClient 다중 인스턴스 경고는 별도 이슈로 남긴다.
- 릴리스: `e281a3e feat(customers): 고객 작업내역 컬럼 설정 보강`을 작업 브랜치에 push하고 Vercel Dev 환경에서 `/customers`를 smoke한다. 신규 SQL·환경변수와 `/landing`, `/demo` 변경은 없다.

### 2026-08-06 프랜차이즈 데모 업무 스토리·실화면 일치 마감 QA

- 첫 진입은 `3분 핵심 체험`과 업무별 시나리오 선택으로 정리했다. 전체 흐름과 가맹 영업 담당자, 출점 개발 담당자, 오픈 준비 담당자, 본부 관리자 관점에서 광고 유입·상담·정보공개서·후보지 연결·물건 기록·계약 준비·가맹 운영·점주 계정과 점주 홈페이지까지 실제 업무 순서로 안내한다.
- 프랜차이즈 사이드바 깊이와 메뉴별 화면은 운영 컴포넌트를 재사용한다. 상세 팝업, 확인창, 물건 기록 패널, 지도 필터, 정보공개서 발송 준비, 후보지/계약 가맹점 연결, 점주 계정 관리와 점주 포털도 샘플 fixture와 메모리 상태로 재현하며 실제 ERP API와 외부 발송은 계속 차단한다.
- 투어 단계 간 목적지 전환이 이전 시나리오의 1단계로 초기화되던 문제를 보정하고, 대상이 화면 밖이나 패널 내부에 있을 때 자동 스크롤·강조 위치를 다시 계산한다. 좁은 화면에서는 설명 카드와 강조 영역이 겹치거나 상세 하단이 잘리지 않도록 viewport 기준 배치를 적용했다.
- 데모 지도는 가짜 도형 기반 배경 대신 운영 지도 컴포넌트를 사용하되, 지도 SDK를 사용할 수 없는 환경에서는 샘플 위치·필터·목록 체험을 유지한다. 본사 요청과 담당자 처리 흐름은 물건 기록 패널의 업무 설명과 요청 상태로 안내한다.
- 데모 첫 화면의 `이 화면 안내`는 최초 체험 선택창을 다시 여는 동작으로 통합했다. 사용자는 전체 흐름을 시작하거나 원하는 업무 시나리오를 선택할 수 있고, 각 화면의 `가이드 열기`에서는 해당 화면 안내를 다시 볼 수 있다.
- 모객 DB 등록/수정 중 같은 연락처가 이미 있을 때 공급자 영문 오류 대신 `같은 연락처로 등록된 가맹 희망자가 이미 있습니다.`를 중앙 알림창에 표시한다. POST와 PUT의 중복 연락처 응답을 같은 상수와 회귀 테스트로 고정했다.
- 자동 검증: 전체 테스트 1,103건, 데모 집중 회귀 32건, 중복 연락처 회귀 6건, `npx tsc --noEmit --pretty false`, `npm run lint -- --quiet`, `npm run build`, `git diff --check`를 통과했다. build에는 기존 workspace root, `baseline-browser-mapping`, Browserslist 경고만 남았다.
- 브라우저 QA: 로컬 데모에서 전체 흐름과 업무별 시나리오의 화면 전환, 딤드 강조, 상세 팝업, 물건 기록, 후보지 연결, 정보공개서, 점주 계정/포털을 확인했다. Next.js 오류 오버레이와 console error는 없었다. 독립 코드·목표·보안·수동 QA·최종 게이트 리뷰가 모두 `APPROVE` 또는 `PASS`였다.
- 릴리스: `4253ecb`, `a2db4cc`, `d848868`, `a1b286c`, `348b518`을 한 배포 단위로 Dev와 production에 반영한다. 신규 SQL과 환경변수 변경은 없으며 `/landing` 변경도 없다.
