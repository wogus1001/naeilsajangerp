...
When writing a contract from a company-uploaded template, `ERP에서 직접 작성` fills template fields inside ERP and sends through the linked UCanSign template. `템플릿에서 직접 작성` opens the saved UCanSign template's sign-request flow directly, so users do not see the generic UCanSign document selection modal again. If the active template version has no linked UCanSign template/document ID, ERP blocks the flow with a clear template-link validation message.
