...
const DEFAULT_INSPECTION_ITEM_DEFINITIONS: readonly Pick<SupervisionInspectionItem, 'id' | 'label'>[] = [
    { id: 'sales-traffic', label: '매출/객수 확인' },
    { id: 'cleanliness', label: '청결' },
    { id: 'service', label: '서비스' },
    { id: 'quality', label: '품질' },
    { id: 'inventory-logistics', label: '재고/물류' },
    { id: 'hq-support', label: '본사 지원 필요사항' }
];
...
export function mergeInspectionItems(input: unknown): readonly SupervisionInspectionItem[] {
    if (!Array.isArray(input)) return buildDefaultInspectionItems();
    ...
    return DEFAULT_INSPECTION_ITEM_DEFINITIONS.map(defaultItem => {
