...
function buildChecklistIssueGroups(
    ...
    checklists.forEach(checklist => {
        ...
        if (!location || checklist.tasks.length === 0) return;
        const key = buildChecklistIssueKey(checklist.tasks);
        const completedTaskIds = completedTaskIdsByLocation.get(checklist.locationId) || new Set<string>();
        const completedCount = checklist.tasks.filter(task => completedTaskIds.has(task.id)).length;
        const current = groups.get(key) || { tasks: checklist.tasks, locations: [] };
        current.locations.push({
            location,
            completedCount,
            totalCount: checklist.tasks.length,
            isComplete: checklist.tasks.length > 0 && completedCount >= checklist.tasks.length
        });
                                        ...
                                        <details className={styles.ownerPortalSubmissionDetails}>
                                            ...
                                            <div className={styles.ownerPortalSubmissionDetailGrid}>
                                                {group.tasks.map((task, index) => (
                                                    <div className={styles.ownerPortalSubmissionDetailItem} key={`${group.key}-${task.id}`}>
                                                        <strong>{String(index + 1).padStart(2, '0')} {task.title}</strong>
                                                        <span>{task.memo || '안내 문구 없음'}</span>
                                                    </div>
                                                ))}
                                            </div>
                                        </details>
