...
export function ApprovalSettingsPage() {
    ...
    async function patchOrganization(patch: ApprovalOrganizationPatch): Promise<boolean> {
        setSaving(true);
        try {
    ...
    async function createDelegation(input: ApprovalDelegationInput): Promise<boolean> {
        setSaving(true);
        try {
        ...
        <section className={styles.page}>
            ...
            {loading ? <div className={styles.loading}>설정을 불러오는 중입니다.</div> : (
                <div className={`${styles.settingsGrid} ${organization.canManageOrganization ? '' : styles.singleColumn}`}>
                    {organization.canManageOrganization && <OrganizationUnitsSection disabled={saving} onDelete={id => void removeOrganizationUnit(id)} onSave={(unit: Partial<ApprovalOrganizationUnit>) => patchOrganization({ units: [unit] })} people={organization.people} units={organization.units} />}
                    {organization.canManageOrganization && <OrganizationMembershipsSection disabled={saving} memberships={organization.memberships} onDelete={id => void removeMembership(id)} onSave={(membership: Partial<ApprovalMembership>) => patchOrganization({ memberships: [membership] })} people={organization.people} units={organization.units} />}
                    <ApprovalDelegationsSection delegations={organization.delegations} disabled={saving} onCreate={createDelegation} onDelete={id => void removeDelegation(id)} people={organization.people} requesterProfileId={organization.requesterProfileId} />
                </div>
            )}
