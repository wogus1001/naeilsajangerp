...
import { fetchAdminFranchiseIntake, promoteAdminLeadRegistration, promoteAdminProperty } from './requests';
import { IntakePropertiesTable } from './IntakePropertiesTable';
import { LeadRegistrationRequestsTable } from './LeadRegistrationRequestsTable';
import { MatchingRequestsTable } from './MatchingRequestsTable';
import type { AdminFranchiseIntakeData, AdminIntakeProperty, AdminLeadRegistrationRequest } from './types';
import styles from './page.module.css';

type IntakeTab = 'properties' | 'leadRegistrations' | 'matchingRequests';
...
type LeadPromotionModalState = {
    readonly lead: AdminLeadRegistrationRequest;
    readonly targetCompanyId: string;
    readonly managerId: string;
};
...
export default function AdminFranchiseIntakePage() {
    ...
    const [leadModal, setLeadModal] = React.useState<LeadPromotionModalState | null>(null);
    ...
    const openLeadPromotionModal = (lead: AdminLeadRegistrationRequest) => {
        setLeadModal({
            lead,
            targetCompanyId: lead.companyId || selectedCompanyId,
    ...
    const leadTargetManagers = React.useMemo(() => {
        if (!leadModal) return [];
        return data.managers.filter(manager => manager.companyId === leadModal.targetCompanyId);
    }, [data.managers, leadModal]);
    ...
    const confirmLeadPromotion = async () => {
        if (!leadModal || !requesterId) return;
        setIsPromoting(true);
        setMessage('');
        try {
            await promoteAdminLeadRegistration({
                leadId: leadModal.lead.id,
                targetCompanyId: leadModal.targetCompanyId,
                managerId: leadModal.managerId || undefined,
                requesterId
            });
            setLeadModal(null);
            setMessage('가맹 희망자를 모객 DB로 반영했습니다.');
            await loadData(selectedCompanyId);
        } catch (error) {
            setMessage(error instanceof Error ? error.message : '모객 DB 반영에 실패했습니다.');
        } finally {
            setIsPromoting(false);
        }
    };
    ...
    return (
        <main className={styles.page}>
            <section className={styles.header}>
                <div>
                    <h1>프랜차이즈 인입 관리</h1>
                    <p>물건 등록 DB, 가맹 희망자 접수, 프랜차이즈 매칭 요청을 회사별로 확인합니다.</p>
                </div>
            </section>
            ...
            {activeTab === 'leadRegistrations' && (
                <LeadRegistrationRequestsTable
                    requests={data.leadRegistrationRequests}
                    requesterId={requesterId}
                    onPromoteAction={openLeadPromotionModal}
                    onSyncedAction={handleSynced}
                    onErrorAction={setMessage}
                />
            )}
            ...
            {leadModal && (
                <div className={styles.modalBackdrop}>
                    <section className={styles.modal}>
                        <h2>모객 DB로 밀어넣기</h2>
                        <p>{leadModal.lead.name} 접수 건을 선택한 회사의 가맹 희망자 목록으로 반영합니다.</p>
                        <label>
                            <span>대상 회사 DB</span>
                            <select
                                value={leadModal.targetCompanyId}
                                onChange={event => setLeadModal(prev => prev ? { ...prev, targetCompanyId: event.target.value, managerId: '' } : prev)}
                            >
                                {data.companies.map(company => <option key={company.id} value={company.id}>{company.name}</option>)}
                            </select>
                        </label>
                        <label>
                            <span>담당자</span>
                            <select value={leadModal.managerId} onChange={event => setLeadModal(prev => prev ? { ...prev, managerId: event.target.value } : prev)}>
                                <option value="">미지정</option>
                                {leadTargetManagers.map(manager => <option key={manager.id} value={manager.id}>{manager.name}</option>)}
                            </select>
                        </label>
                        <div className={styles.modalActions}>
                            <button className={styles.secondaryButton} onClick={() => setLeadModal(null)} disabled={isPromoting}>취소</button>
                            <button className={styles.primaryButton} onClick={confirmLeadPromotion} disabled={isPromoting}>{isPromoting ? '반영 중' : '모객 DB 반영'}</button>
                        </div>
                    </section>
                </div>
            )}
