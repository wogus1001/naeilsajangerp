...
export async function listSchedules(
    ...
    const dateFrom = cleanString(searchParams.get('dateFrom')) || cleanString(searchParams.get('from'));
    const dateTo = cleanString(searchParams.get('dateTo')) || cleanString(searchParams.get('to'));
...
export async function createSchedule(
    ...
    const { data, error } = await supabaseAdmin
        ...
        .insert({
            ...
            color: cleanString(body.color) || '#3182f6',
