...
async function saveDraft(
    ...
    throwDatabaseError(error);
    return data
        ? ok({ document: documentView(data), action: 'saveDraft' })
        : fail(409, 'CONFLICT', 'The document status changed before the draft was saved');
}
...
export async function handleApprovalActionPOST(
    ...
    try {
        ...
        if (action === 'submit' && document.template_id) {
            const resubmissionVersion = document.current_version_id && ['반려', '회수'].includes(document.status)
                ? await context.supabase.from('approval_document_versions')
                    .select('template_version_id')
                    .eq('id', document.current_version_id)
                    .eq('company_id', context.companyId)
                    .maybeSingle<{ readonly template_version_id: string | null }>()
                : { data: null, error: null };
            throwDatabaseError(resubmissionVersion.error);
            const template = resubmissionVersion.data?.template_version_id
                ? { data: { current_version_id: resubmissionVersion.data.template_version_id }, error: null }
            ...
            throwDatabaseError(template.error);
            const { data: version, error: versionError } = template.data?.current_version_id
                ? await context.supabase.from('approval_template_versions')
                    .select('fields')
                    .eq('id', template.data.current_version_id)
                    .eq('company_id', context.companyId)
