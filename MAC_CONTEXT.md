...
- 프랜차이즈 본사용 고도화 방향
  - 1순위: 모객 전환율 개선 Command Center
    ...
    - 2026-06-12 모객 DB 구조 분리 진행: `page.tsx`에서 후보자 등록 모달, 빠른 상담 이력 모달, 후보자 상세 패널, Meta 연동 설정 패널을 `src/components/franchise/leads` 컴포넌트로 분리했다. 검증은 `npx tsc --noEmit --pretty false`, `npm run lint -- --quiet`, 관련 lead/checklist 테스트, `git diff --check`, `npm run build` 통과. 다음 리팩터링은 Meta/엑셀/고객전환/후보지 연결 action hook 분리다.
  - 2순위: 본사 운영관리
