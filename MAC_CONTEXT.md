...
- 하위 에이전트는 사용자가 "하위 에이전트", "병렬", "subagent"를 명시한 경우에만 새로 연다. 기본 작업, 코드 수정, 문서 정리, 단일 QA는 메인 세션에서 직접 처리한다.
- 이미 열린 하위 에이전트가 같은 맥락의 후속 작업에 재사용 가능하면 새로 열기 전에 `send_input` 재사용을 먼저 검토한다.
- 코드리뷰/QA/탐색용 하위 에이전트는 결과를 회수한 뒤 바로 닫아 동시 실행 수와 세션 혼잡을 줄인다. 여러 개를 열어야 할 때도 역할을 `코드리뷰 1개`, `QA 1개`처럼 최소 단위로 제한한다.
- 하위 에이전트나 worker에게 `MAC_CONTEXT.md` 수정 권한을 주지 않는다. 이 파일은 메인 세션에서만 수정한다.
...
- 프랜차이즈 본사용 고도화 방향
  - 1순위: 모객 전환율 개선 Command Center
    ...
    - 2026-07-01 업체 계약함 MVP 진행: `/contracts/vendor`에서 업체 계약 파일/전자계약 연결과 D-30/D-7 만료 알림을 관리한다. 적용 SQL은 `ERP/web/supabase_franchise_vendor_contracts_migration.sql`이며 사용자가 Supabase SQL Editor에 직접 등록해야 한다.
...
- 2026-06-16 회사별 메뉴 관리에서 기본 대시보드 타입을 지정하도록 추가했다. 기본값은 `A 타입`이고, `B 타입`은 기존 일정/계약/점포/고객 요약 화면이다. `/dashboard`의 `대시보드 보기` 토글은 제거했으며 회사 설정값으로 렌더링한다.
